...
Project lives at `/mnt/storage/NAS/Jarvis/cmd/` with a symlink `~/cmd → /mnt/storage/NAS/Jarvis/cmd`.
Git remote: `http://10.0.0.58:3000/Grindlewalt/Jarvis.git`

Only `server.py` is root-owned — it must be staged via `/tmp/` then `sudo cp`'d. All other files SCP directly to `~/cmd/`.

**After every file change: SCP → then commit and push on the server.**
...
# Then commit the changes on the server
mcssh "cd ~/cmd && git add -A && git commit -m 'update: <description>' && git push"
```

## Git Workflow

Every change to the project must be committed. After deploying files:

```bash
mcssh "cd ~/cmd && git add -A && git commit -m 'description' && git push"
```

Note: `server.py` is root-owned. To include it in git: `mcssh "cd ~/cmd && sudo git add server.py"`
